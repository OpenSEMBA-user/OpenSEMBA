...
PhysicalModelGroup::PhysicalModelGroup(
		...
		const vector<PMVolumePML*> pml, const vector<PMSurface*>& surface,
...
PhysicalModelGroup::createAndAssignPML(
 ...
 MeshVolume* mesh) {
	...
	pair<CVecD3,CVecD3> bound, pmlBound;
	bound = mesh->getBound(internalBorder);
	for (unsigned int i = 0; i < 3; i++) {
		if (direction[i] == PMVolumePML::minus) {
			pmlBound.second(i) = bound.first(i);
			//pmlBound.first(i) = - INFINITY;
				pmlBound.first(i) = - numeric_limits<double>::infinity() ;
		} else if (direction[i] == PMVolumePML::plus) {
			pmlBound.first(i) = bound.second(i);
			pmlBound.first(i) = numeric_limits<double>::infinity() ;
		} else {
			pmlBound.first(i) = bound.first(i);
			pmlBound.second(i) = bound.second(i);
		}
	}
	...
	if (PMLIds.size() != 0) {
		pmlBound = mesh->getBound(PMLIds);
		unsigned int lastId = count();
		pml_.push_back(new PMVolumePML(lastId+1, direction, pmlBound));
		updatePointers();
