...
template<class T>
const Real Surface<T>::curvatureTolerance = 1e-12;

template<class T>
Surface<T>::Surface(const LayerId layerId,
                    const MatId   matId)
...
template<class T>
Surface<T>::Surface(const ElementId id,
...
template<class T>
Surface<T>::Surface(const Surface& rhs)
:   Element<T>(rhs) {
