...
public:
	...
	Element*
	 getPtrToId(const unsigned int id);
	const Element*
	 getPtrToId(const unsigned int id) const;
	const Tri*
	 getTriPtrToId(const unsigned int id) const;
	const Tet*
	 getTetPtrToId(const unsigned int id) const;
	void
	 linearize();
	bool
	 isLinear() const;
	bool
	 areTetrahedrons(const vector<unsigned int>& elemIds) const;
	bool
	 areTriangles(const vector<unsigned int>& elemIds) const;
	vector<unsigned int>
	 getHexIds() const;
	void
	 removeHex();
