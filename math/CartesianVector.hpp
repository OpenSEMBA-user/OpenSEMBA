...
CartesianVector<T,D>::norm() const {
	double sum = 0;
	for (unsigned int i = 0; i < D; i++) {
		sum += (double) std::fabs((double) val[i]) * std::fabs((double) val[i]);
	}
	return sqrt(sum);
}
...
CartesianVector<T,D>::operator==(
 const CartesianVector<T, D>& param) const {
	for (int i = 0; i < D; i++) {
		if (std::fabs((double) (val[i] - param.val[i])) >= tolerance) {
			return false;
		}
	}
	return true;
...
CartesianVector<T,D>::isContainedInPlane(
 const CartesianPlane plane) const {
	...
	switch (plane) {
	case xy:
		if (std::fabs((T) val[2]) < numeric_limits<T>::epsilon()*1e8) {
			return true;
		}
		break;
	case yz:
		if (std::fabs((T) val[0]) < numeric_limits<T>::epsilon()*1e8) {
			return true;
		}
		break;
	case zx:
		if (std::fabs((T) val[1]) < numeric_limits<T>::epsilon()*1e8) {
			return true;
		}
		break;
	}
