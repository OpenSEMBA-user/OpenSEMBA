...
public:
   StaMatrix();
   StaMatrix(const StaMatrix<T,NROWS,NCOLS>& rhs);
   ~StaMatrix();
   ...
   UInt nRows() const;
   UInt nCols() const;
   StaMatrix<T,NROWS,NCOLS>& operator=(const StaMatrix<T,NROWS,NCOLS>& rhs);
   StaMatrix<T,NROWS,NCOLS>& operator=(const DynMatrix<Int>& rhs);
   StaMatrix<T,NROWS,NCOLS>& assign(const StaMatrix<Int,NROWS,NCOLS>& rhs);
   StaMatrix<T,NROWS,NCOLS>& operator=(const DynMatrix<Real>& rhs);
   StaMatrix<T,NROWS,NCOLS>& operator+=(const Real rhs);
   StaMatrix<T,NROWS,NCOLS>& operator+=(const StaMatrix<T,NROWS,NCOLS>& rhs);
   StaMatrix<T,NROWS,NCOLS>& operator-=(const StaMatrix<T,NROWS,NCOLS>& rhs);
   StaMatrix<T,NROWS,NCOLS>& operator*=(const T rhs);
   StaMatrix<T,NROWS,NCOLS>& operator/=(const T rhs);
   StaMatrix<T,NROWS,NCOLS> operator*(T rhs) const;
   CartesianVector<T,NCOLS> operator*(CartesianVector<T,NCOLS> rhs) const;
   StaMatrix<T,NROWS,NCOLS> operator+(StaMatrix<T,NROWS,NCOLS> &rhs) const;
   StaMatrix<T,NCOLS,NROWS>& invert();
   StaMatrix<T,NCOLS,NROWS> transpose();
   void copy(vector<vector<T> > values);
   bool operator==(const StaMatrix<T,NROWS,NCOLS>& rhs) const;
   bool operator<(const StaMatrix<T,NROWS,NCOLS>& rhs) const;
...
StaMatrix<T,NROWS,NCOLSB> operator*(
        const StaMatrix<T,NROWS,NCOLS>& lhs,
