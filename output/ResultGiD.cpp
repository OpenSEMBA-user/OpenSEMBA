...
ResultGiD::ResultGiD(
      ...
      int& elemCounter,
      const DG* dg,
      const Mesh* mesh) : OutputRequest(*p) {
   dg_ = dg;
   mesh_ = mesh;
   ...
   case Element::VOLUME:
      ...
      break;
   default:
      cerr << "ERROR @ GiDResult ctor:" << endl;
      assert(false);
      break;
   }
}
...
ResultGiD::getOutputValueFromFields(
   ...
   case OutputRequest::magneticField:
      ...
      break;
   default:
      cerr << "ERROR @ ResultGiD::getOutputValueFromFields: "
      << "Not implemented for output type: " << outputTypeStr() << endl;
      return CVecD3();
   }
}

GiD_ResultType ResultGiD::getGiDResultType() const {
   switch (getOutputType()) {
   case OutputRequest::electricField:
...
GiD_ResultLocation
ResultGiD::getGiDResultLocation() const {
