...
template <class T>
DynMatrix<T>& DynMatrix<T>::operator=(const vector<T>& param) {
...
template <class T>
void DynMatrix<T>::copy(const DynMatrix<Real>& param) {
    _nRows = param.nRows();
    _nCols = param.nCols();
...
template<class T>
DynMatrix<T>& DynMatrix<T>::sortAndRemoveRepeatedRows_omp() {
...
template<class T>
void DynMatrix<T>::sortAndRemoveRepeatedRows() {
    this->sortRows(0, nCols());
    removeRepeatedSortedRows();
}

template<class T>
void DynMatrix<T>::removeRepeatedSortedRows() {
    vector<vector<T> > aux;
    aux.reserve(nRows());
    UInt nonRepeated = 0;
    UInt nCompared;
...
template <class T>
DynMatrix<T>& DynMatrix<T>::operator=(const T &param) {
    for (UInt i = 0; i < _nRows; i++)
        for (UInt j = 0; j < _nCols; j++)
            _val[i][j] = param;
    return *this;
...
template <class T>
inline T DynMatrix<T>::operator() (const UInt row, const UInt col) const {
    assert(row < _nRows && col < _nCols);
    return _val[row * _nCols + col];
}

template <class T>
inline T& DynMatrix<T>::operator()(const UInt row, const UInt col) {
    assert(col < _nCols);
    assert(row < _nRows);
    return _val[row * _nCols + col];
}
...
template <class T>
inline T DynMatrix<T>::val(const UInt i) const {
    assert(i < _nRows * _nCols);
    return _val[i];
}

template <class T>
inline T& DynMatrix<T>::val(const UInt i) {
    assert(i < _nRows * _nCols);
    return _val[i];
}

template <class T>
inline T DynMatrix<T>::val(const UInt row, const UInt col) const {
    assert(col < _nCols);
    assert(row < _nRows);
    return _val[row * _nCols + col];
}
...
template <class T>
inline T& DynMatrix<T>::val(const UInt row, const UInt col) {
    assert(col < _nCols);
    assert(row < _nRows);
    return _val[row * _nCols + col];
}
...
template <class T>
const T* DynMatrix<T>::valPtr(const UInt row, const UInt col) const {
    assert(row < _nRows && col < _nCols);
    return &_val[row * _nCols + col];
}

template <class T>
DynMatrix<T> DynMatrix<T>::operator*(DynMatrix<T> &param) const {
...
template <class T>
DynMatrix<T> DynMatrix<T>::operator*(const T param) const {
    DynMatrix<T> res(_nRows, _nCols);
    for (UInt i = 0; i < _nRows; i++) {
        for (UInt j = 0; j < _nCols; j++) {
...
template<class T>
DynMatrix<T> DynMatrix<T>::eliminateColumns(
...
template <class T>
bool DynMatrix<T>::operator==(const DynMatrix<T>& param) const {
    bool res = true;
...
template <class T>
DynMatrix<T> DynMatrix<T>::convolute(const DynMatrix<T> &param) const {
    // PURPOSE:
...
template <class T>
void DynMatrix<T>::printInfo() const {
    printInfo(_nRows, _nCols);
}

template <class T>
void DynMatrix<T>::printInfo(UInt rows, UInt cols) const {
    UInt i, j;
...
template <class T>
DynMatrix<T> DynMatrix<T>::reshape(UInt rows, UInt cols) {
...
template <class T>
DynMatrix<T> DynMatrix<T>::reshape(UInt rows, UInt cols, Int order) {
...
template<class T>
DynMatrix<T> DynMatrix<T>::kron(DynMatrix<T>& rhs) const {
...
template <class T>
DynMatrix<T> DynMatrix<T>::invert() {
...
template <class T>
void DynMatrix<T>::mergeSortedRows(
        const DynMatrix<T>& a,
        const UInt iCol,
        const UInt lCol) {
    assert(this != &a);
...
template <class T>
DynMatrix<T>& DynMatrix<T>::transpose() {
