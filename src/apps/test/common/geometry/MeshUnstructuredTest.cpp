...
TEST_F(GeometryMeshUnstructuredTest, ctor) {
    ...
    EXPECT_EQ(mesh_.elems().getGroupOf<Tetrahedron>().size(), 1);
    EXPECT_EQ(mesh_.elems().getGroupOf<Triangle>().size(), 1);
}

TEST_F(GeometryMeshUnstructuredTest, matchingFaces) {
    const Tetrahedron4* tet = mesh_.elems().getGroupOf<Tetrahedron4>()(0);
    vector<Face> faces;
    ...
    const Triangle3* tri = mesh_.elems().getGroupOf<Triangle3>()(0);
