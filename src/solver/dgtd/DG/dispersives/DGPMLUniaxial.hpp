...
template<Int D>
void DGPMLUniaxial<D>::initUniaxial(
...
#pragma omp parallel for private(i,j,e)
        ...
        for (e = 0; e < nElem; e++) {
            if (e1 <= elem[e] && elem[e] < e2) {
                ...
                add_am_v_prod<Real,np,np>(&rhsE(dir1)[j], sig1[e], &E(dir1)[j], Constants::eps0);
...
template<Int D>
void DGPMLUniaxial<D>::computelRHSElectricPolarizationCurrents(
        const FieldR3& E,
        const UInt e1, const UInt e2) {
    if (useConstantConductivity) {
        UInt i, j, e, n;
