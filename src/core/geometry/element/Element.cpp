...
const Coordinate::Coordinate<T,3>* Element<T>::getMinV() const {
    ...
    for (Size i = 1; i < numberOfVertices(); i++) {
...
std::vector<const Coordinate::Coordinate<T,3>*> Element<T>::getSideCoordinates(
    ...
    for (Size i = 0; i < numberOfSideCoordinates(); i++) {
...
template<class T>
void Element<T>::setV(const Size i, const Coordinate::Coordinate<T,3>* coord) {
    printInfo();
...
template class Element<Math::Real>;
template class Element<Math::Int >;
