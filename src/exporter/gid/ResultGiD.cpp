...
void Output::writePointProbeMesh(Int& coordCounter, Int& elemCounter) {
...
void Output::writeTetProbeMesh(Int& coordCounter, Int& elemCounter) {
   UInt nV;
...
CVecR3 Output::getOutputValueFromFields(
   ...
   case OutRq::magneticField:
      ...
      break;
   default:
      cerr << endl << "ERROR @ Output::getOutputValueFromFields: "
      << "Not implemented for output type: " << outputTypeStr() << endl;
      return CVecR3();
   }
}

string Output::getGiDGaussPointType() const {
...
vector<string> Output::getComponentNames() const {
   string letter;
   switch (getOutputType()) {
   case OutRq::electricField:
      letter = 'E';
